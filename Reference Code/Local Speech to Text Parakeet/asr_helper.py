...
# Add parent directory to path for imports (src.memory, src.types)
...
FILLER_WORDS = r"\b(um|uh|er|ah|hmm|hm|mhm|uh-huh|like,?)\b"
...
def mechanical_cleanup(text: str) -> str:
    ...
    Performs:
    - Remove filler tokens (um, uh, er, ah, like)
    - Normalize whitespace (repeated spaces -> single)
    - Fix broken line breaks (join mid-sentence splits)
    ...
    # 1. Remove filler tokens
    text = re.sub(FILLER_WORDS, "", text, flags=re.IGNORECASE)

    # 2. Normalize whitespace (repeated spaces -> single)
    text = re.sub(r" {2,}", " ", text)

    # 3. Fix broken line breaks (single newline mid-sentence -> space)
    # Matches: lowercase/comma followed by newline followed by lowercase
    text = re.sub(r"(?<=[a-z,])\n(?=[a-z])", " ", text)

    # 4. Remove consecutive duplicate words (stutters like "the the")
    text = re.sub(r"\b(\w+)(\s+\1)+\b", r"\1", text, flags=re.IGNORECASE)

    # 5. Remove repeated short phrases (2-3 words repeated)
    text = re.sub(r"\b((\w+\s+){1,3})\1+", r"\1", text)

    # 6. Clean up any leftover multiple spaces from removals
    text = re.sub(r" {2,}", " ", text)
...
def audio_has_significant_energy(wav_path: Path, rms_threshold: float = 0.01) -> bool:
    """
    Return True if the audio has RMS above threshold (non-silent).
    Used to distinguish silent recordings from noisy/static audio when transcript is empty.
    """
    try:
        data, _ = sf.read(str(wav_path))
        if data.size == 0:
            return False
        # Flatten to one channel if stereo
        if data.ndim > 1:
...
def chunk_audio_vad(
    ...
    speech_segments = get_speech_segments(wav_path)
    ...
    # Load audio once into memory for fast slicing (no subprocess per chunk)
    print("  Loading audio into memory for fast chunking...")
...
def _word_similarity(words1: list[str], words2: list[str]) -> float:
    """
    Calculate similarity between two word lists using SequenceMatcher.
...
def fuzzy_dedup_overlap(
    ...
    - Use fuzzy matching (SequenceMatcher) to handle ASR variations
...
def cleanup_transcript_pipeline(
    ...
    # Stage 1: Mechanical cleanup on each chunk
    print("Stage 1: Mechanical cleanup...")
    cleaned_transcripts = [
        (mechanical_cleanup(text), start, end) for text, start, end in transcripts
    ]
